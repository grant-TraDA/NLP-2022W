...
def parse_args():
    ...
    parser.add_argument(
        "--input_path", "-i", type=Path, help="Path to recipeNLG dataset .csv"
    )
    parser.add_argument(
        "--output_path",
        "-o",
        type=Path,
        help="Path to output .csv with nutritional information",
    )
...
def main(args):
    ...
    for ingredient in tqdm(ingredients):
        ...
        while response.status_code != 200:
            print(
                f"Can't get proper response... waiting | status code: {response.status_code}"
            )
            time.sleep(60)
            response = requests.get(url)
        ...
        try:
            parsed_nutrients = {
                nutrient_name: False for nutrient_name in nutrient_ids.values()
            }
            ...
            for nutrient in best_food_data["foodNutrients"]:
                if nutrient["nutrientId"] in nutrient_ids.keys():
                    ...
                    ingredient_nutrients[f"{nutrient_name}_unit"] = nutrient[
                        "unitName"
                    ]
        ...
        new_df = pd.DataFrame(
            data=[ingredient_nutrients], columns=ret_df_columns
        )
